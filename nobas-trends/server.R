...
server <- function(input, output, session) {
  ...
  HWSTCOLS <- c("HWST","hwst","Hwst","Hot Water Supply Temperature","K-Type","Temp","Temperature","TEMP")
  HWSTSPCOLS <- c("HWSTSP","hwstsp","Hwstsp","Hot Water Supply Temperature Setpoint","Temp","Temperature","TEMP")
  HWRTCOLS <- c("HWRT","hwrt","Hwrt","Hot Water Return Temperature","K-Type","Temp","Temperature","TEMP")
  ...
  CHWSTCOLS <- c("CHWST","chwst","Chwst","Temp","Chilled Water Supply","K-Type","Temp","Temperature","TEMP")
  CHWSTSPCOLS <- c("CHWSTSP","chwstsp","Chwstsp","Set Point","Chilled Water Supply Set Point")
  CHWRTCOLS <- c("CHWRT","chwrt","Chwrt","Temp","Chilled Water Return","K-Type","Temp","Temperature","TEMP")
  ...
  CRTCOLS <- c("CRT","crt","Crt","Condensate Return Temperature","Temp","Temperature","TEMP")
  BMSCOLS <- c("BMS","bms","Bms","Burner Motor Status", "Motor","Fan")
  STACKCOLS <- c("Stack Temperature","Stack","K-Type","Temp","Temperature","TEMP")
  ZTCOLS <- c("ZT","zt","Zt","Zone Temperature","Zone Temp","Temp","Temperature","TEMP")
  ZTSPCOLS <- c("ZTSP","ztsp","Ztsp","Zone Temperature Setpoint","Zone Temp Setpoint","Temp","Temperature","TEMP")
  ZTSPACOLS <- c("ZTSPA","ztspa","Ztspa","Zone Temperature Setpoint Air","Zone Temp Setpoint Air System","Temp","Temperature","TEMP")
  ZTSPPCOLS <- c("ZTSP","ztsp","Ztsp","Zone Temperature Setpoint","Zone Temp Setpoint Perimeter","Temp","Temperature","TEMP")
  ZTSBSPCOLS <- c("ZTSBSP","ztsbsp","Ztsbsp","Zone Temperature Setback Setpoint","Zone Temp Setback Setpoint","Temp","Temperature","TEMP")
  ...
  observeEvent(input$trend2aTab,{
    updateTabItems(session,'tabs',selected = "trend2a")
    js$ScrollUp()
  })
  ...
  observeEvent(input$OccToAHUTab,{
    updateTabItems(session,'tabs',selected = "OccToAHU")
    js$ScrollUp()
  })
  ...
  observeEvent(input$OccToCHPTab,{
    updateTabItems(session,'tabs',selected = "OccToCHP")
    js$ScrollUp()
  })
  ...
  observeEvent(input$OccToBPTab,{
    updateTabItems(session,'tabs',selected = "OccToBP")
    js$ScrollUp()
  })
  ...
  observeEvent(input$OccToZTab,{
    updateTabItems(session,'tabs',selected = "OccToZ")
    js$ScrollUp()
  })
  ...
  observeEvent(input$OccToPTab,{
    updateTabItems(session,'tabs',selected = "OccToP")
    js$ScrollUp()
  })
  ...
  oad_trend_t2a <- callModule(csvFile,"OAD2a",
  ...
  all_data_t2a <- reactive({
    list(fan_trend_t2a(),oad_trend_t2a())
  })
  
  date_range_t2a <- callModule(dateRange,"placeholder-id",data=all_data_t2a)
  occupancyRects_t2a <- callModule(occupancy,"occ2a",date_range_t2a)
  
  callModule(plotting,"trend2a",data=all_data_t2a,occupancyRects=occupancyRects_t2a,y1label='Fan Status')
  #####end of trend 2a######
  ...
  ctfs_trend_t15 <- callModule(csvFile,"CTFS15",
                             targetColumns=reactive({CTFSCOLS}),
                             stateChange=reactive({TRUE}),
                             periodicity15=reactive({TRUE}),
  ...
  crt_trend_t17 <- callModule(csvFile,"CRT17",
                               targetColumns=reactive({CRTCOLS}),
                               stateChange=reactive({FALSE}),
                               periodicity15=reactive({TRUE}),
                               name=reactive({"CRT"}),
                               color=reactive({"red"}),
  ...
  ######start of trend 22#######
   ...
   bms_trend_t22 <- callModule(csvFile,"BMS22",
                              targetColumns=reactive({BMSCOLS}),
                              stateChange=reactive({TRUE}),
                              periodicity15=reactive({FALSE}),
   ...
   stack_trend_t22 <- callModule(csvFile,"STACK22",
                              targetColumns=reactive({STACKCOLS}),
                              stateChange=reactive({FALSE}),
                              periodicity15=reactive({TRUE}),
  ...
  ########start of trend 23#######
   ...
   hwstsp_trend_t23 <- callModule(csvFile,"HWSTSP23",
                                 targetColumns=reactive({HWSTSPCOLS}),
                                 stateChange=reactive({FALSE}),
                                 periodicity15=reactive({TRUE}),
                                 name=reactive({"HWSTSP"}),
                                 color=reactive({"orange"}),
                                 axis=reactive({'y1'}))
 ...
 ####end of trend 23#######
  ...
  ####start of trend 26######
   ...
   ztsp_trend_t26 <- callModule(csvFile,"ZTSP26",
                                 targetColumns=reactive({ZTSPCOLS}),
                                 stateChange=reactive({FALSE}),
                                 periodicity15=reactive({TRUE}),
                                 name=reactive({"ZTSP"}),
                                 color=reactive({"red"}),
   ...
   ztsbsp_trend_t26 <- callModule(csvFile,"ZTSBSP26",
                                 targetColumns=reactive({ZTSBSPCOLS}),
                                 stateChange=reactive({FALSE}),
                                 periodicity15=reactive({TRUE}),
  ...
  ######start of trend 2737a#######
   ...
   boiler_trend_t2737a <- callModule(csvFile,"Boiler-t2737a",
                                     targetColumns=reactive({BOILERCOLS}),
                                     stateChange=reactive({TRUE}),
                                     periodicity15=reactive({TRUE}),
  ...
  ########start of trend 27b#######
   ...
   ztspa_trend_t27b <- callModule(csvFile,"ZTSP-A27b",
                               targetColumns=reactive({ZTSPACOLS}),
                               stateChange=reactive({FALSE}),
                               periodicity15=reactive({TRUE}),
  ...
  #######start of trend 28b#######
   ...
   ratsp_trend_t30b <- callModule(csvFile,"RATSP30b",
                                targetColumns=reactive({RATSPCOLS}),
                                stateChange=reactive({FALSE}),
                                periodicity15=reactive({TRUE}),
                                name=reactive({"RATSP"}),
                                color=reactive({"yellow"}),
   ...
   ztspp_trend_t36b <- callModule(csvFile,"ZTSP-P36b",
                                  targetColumns=reactive({ZTSPPCOLS}),
                                  stateChange=reactive({FALSE}),
                                  periodicity15=reactive({TRUE}),
                                  name=reactive({"ZTSP-P"}),
                                  color=reactive({"#ba475a"}),
